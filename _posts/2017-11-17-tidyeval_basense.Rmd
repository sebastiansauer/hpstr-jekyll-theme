...
framework to dealing with non-standard evaluation (NSE) in R. In short, NSE
is about capturing some R-code, withholding execution, maybe editing the code,
and finally executing it later and/or somewhere else.
...
In addition, most of the knowledge is derived from Hadley Wickham's book [Advanced R](http://adv-r.had.co.nz/).
...
Does work. With tidyeval, the environment of the variables are memorized. With base R, the environment is forgotten.
...
What's that for a strange environment? Well, that's the NAMESPACE of package `base`,  the exported objects of that package. A namespace is useful so the search path will not be cluttered by too many objects. Using a namespace we can define which objects should be made (easily) accessible to the users.

But in this environment, for sure, we will not have put some variable.  How comes that this code does work:
...
We see that each object has an environment, and each environment has exactly one "parent" environment - with the notably exception of the `emptyenv` which is the first "ancestor" of this sequence.
...
NSE and tidyeval in particular and intriguing and quite complex aspects of the R language. I am still trying to get my head around it. Let me know your toughs and feel free to point out new or diverging ideas to this post.
